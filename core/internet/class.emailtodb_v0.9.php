 ...
 ******************************************************************************/
  ...
  function spam_detect(){
    ...
    $execute = mysqli_query($db,"SELECT ID, IDEmail, EmailFrom, EmailFromP, EmailTo, Subject, Message, Message_html FROM emailtodb_email WHERE ID='".$id."'");
  ...
  function check_blacklist($email){
   #spam - emails 
   $execute = mysqli_query($db,"SELECT Email FROM emailtodb_list WHERE Email='".addslashes($email)."' AND Type='B'");
   $row = mysqli_fetch_array($execute);
   $e_mail = $row['Email'];
   if($e_mail == $email){
    return 1;
    ...
    $execute = mysqli_query($db,"SELECT Word FROM emailtodb_words ");
...
*/
 ...
 $execute = mysqli_query($db,"INSERT INTO emailtodb_email (IDEmail, EmailFrom, EmailFromP, EmailTo, DateE, DateDb, Subject, MsgSize) VALUES
      ('".$message_id."',
      '".$email['FROM_EMAIL']."',
  ...
  $execute = mysqli_query($db,"select LAST_INSERT_ID() as UID");
  $row = mysqli_fetch_array($execute);
  $this->newid = $row["UID"];
...
$execute = mysqli_query($db,"INSERT INTO emailtodb_attach (IDEmail, FileNameOrg, Filename) VALUES
          ('".$this->newid."',
...
*/
  ...
  if($type == 'PLAIN') $execute = mysqli_query($db,"UPDATE emailtodb_email SET Message='".addslashes($msg)."' WHERE ID= '".$this->newid."'");

  if($type == 'HTML')  $execute = mysqli_query($db,"UPDATE emailtodb_email SET Message_html='".addslashes($msg)."' WHERE ID= '".$this->newid."'");
 ...
 */
  function add_db_log($email, $info){
    ...
    $execute = mysqli_query($db,"INSERT INTO emailtodb_log (IDemail, Email, Info, FSize, Date_start, Status) VALUES
      ('".$this->newid."',
      '".$email['FROM_EMAIL']."',
 ...
 */
  function update_folder($id, $folder){
    ...
    $execute = mysqli_query($db,"UPDATE emailtodb_email SET Type = '".addslashes($folder)."' WHERE ID = '".$id."'");
 ...
 */
  function update_db_log($info, $id){
    ...
    $execute = mysqli_query($db,"UPDATE emailtodb_log  SET Status = '1', Info='".addslashes(strip_tags($info))."', Date_finish = '".date("Y-m-d H:i:s")."' WHERE IDlog = '".$id."'");
 ...
 */
  ...
  $email = array();
   ...
   $execute = mysqli_query($db,"SELECT IDlog, IDemail, Email, Info, FSize, Date_start, Date_finish, Status FROM emailtodb_log ORDER BY Date_finish DESC LIMIT 100");
   while($row = mysqli_fetch_array($execute)){
    $ID = $row['IDlog'];
 ...
 */
  ...
  $email = array();
   ...
   $execute = mysqli_query($db,"SELECT ID, IDEmail, EmailFrom, EmailFromP, EmailTo, DateE, DateDb, Subject, Message, Message_html, MsgSize FROM emailtodb_email ORDER BY ID DESC LIMIT 25");
   while($row = mysqli_fetch_array($execute)){
    $ID = $row['ID'];
