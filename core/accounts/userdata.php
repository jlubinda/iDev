...
else
{
	function userDetails1($user){
		...
		$sel = "SELECT * FROM _user_acnts WHERE userID = '".$user."' OR Email = '".$user."' OR vCode = '".$user."' OR Mobile = '".standadizesMobile($user)."';";
		//echo $sel."<br><br>";
...
{
	function checkUserHasVCode($userID){
		
		include find_file("cnct.php");

		$insd = "SELECT vCode FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
...
else
{
	function userID($id){
		...
		$query = "SELECT max(userID) AS maxID FROM _user_acnts WHERE (Mobile = '".standadizesMobile($id)."' OR vCode = '".$id."' OR Email = '".$id."' OR userID = '".$id."' );";
		//echo $query."<br>";
...
{
	function setUserVCode($userID){
		
		$vCode = md5(uniqueCode()."|".getMobile($userID)."|".getEmail($userID)."|".userID($userID));
		
		include find_file("cnct.php");

		$insd = "UPDATE _user_acnts SET vCode = '".$vCode."' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function userData(){
		...
		if(chkSes()=="Active")
		{
			$SesVar = SesVar();
			include find_file("cnct.php");
			$ses = explode("sess",$SesVar);
			$online = "SELECT * FROM online WHERE _sessid = '".$ses[1]."';";
			$online_res = mysqli_query($db,$online);
			$row_online = mysqli_fetch_array($online_res);
			$uid = $row_online["_idcry"];
...
{
	function userDetails($user="",$limiter="",$store="",$product="",$category=""){
...
else
{
	function getUserData($userID,$type){
		...
		$sel = "SELECT * FROM _user_acnts WHERE userID = (SELECT max(userID) FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."'));";
		//echo $sel;
		@$res = mysqli_query($db,$sel);
		@$num = mysqli_num_rows(@$res);
...
else
{
	function checkAccount($type,$userAccount,$password=""){
		...
		if($type=="CHECK")
		{
			...
			$query = "SELECT * FROM _user_acnts WHERE (userID = '".$userAccount."' OR Mobile = '".$Mobile."' OR vCode = '".$userAccount."' OR Email = '".$userAccount."') AND (Active = '1' OR Active = 'Active' OR Active = 'Yes');";
		}
		elseif($type=="CHECK ALL")
		{
			// get a customer from customer table
			$query = "SELECT * FROM _user_acnts WHERE (userID = '".$userAccount."' OR Mobile = '".$Mobile."' OR vCode = '".$userAccount."' OR Email = '".$userAccount."');";
		}
		elseif($type=="CHECK LOGIN")
		{
			$query = "SELECT * FROM _user_acnts WHERE (Mobile = '".$Mobile."' OR Email = '".$userAccount."') AND  Password = '".md5($password)."' AND (Active = '1' OR Active = 'Active' OR Active = 'Yes');";
		}
		elseif($type=="LOGIN")
		{
			$query = "SELECT * FROM _user_acnts WHERE (Mobile = '".$Mobile."' OR Email = '".$userAccount."') AND  Password = '".md5($password)."' AND (Active = '1' OR Active = 'Active' OR Active = 'Yes');";
		...
		elseif($type=="TEMP")
		{
			$query = "SELECT * FROM _user_acnts WHERE (Mobile = '".$Mobile."' OR Email = '".$userAccount."') AND Active = 'Temp';";
		}
		elseif($type=="SUSPENDED")
		{
			$query = "SELECT * FROM _user_acnts WHERE (userID = '".$userAccount."' OR Mobile = '".$Mobile."' OR vCode = '".$userAccount."' OR Email = '".$userAccount."') AND Active = 'Suspended';";
		}
		elseif($type=="STORE")
		{
			$query = "SELECT * FROM _user_acnts WHERE (userID = '".$userAccount."' OR Mobile = '".$Mobile."' OR vCode = '".$userAccount."' OR Email = '".$userAccount."') AND (Active = '1' OR Active = 'Active' OR Active = 'Yes') AND enableStore = '1';";
		}
		...
		if ($num>=1) 
		{
			...
			elseif($type=="LOGIN")
			{
				...
				$customercheck = array();
				$customercheck["fName"] = $row["FirstName"];
				$customercheck["lName"] = $row["LastName"];
				$customercheck["gender"] = $row["sex"];
				$customercheck["dob"] = $row["DOB"];
				$customercheck["houseNo"] = $row["houseNo"];
				//$customercheck["username"] = $row["username"];
				$customercheck["street"] = $row["street"];
				$customercheck["area"] = $row["area"];
				$customercheck["phone"] = $row["Mobile"];
				$customercheck["email"] = $row["Email"];
				$customercheck["nrcNumber"] = $row["nrcNumber"];
				$customercheck["town"] = $row["town"];
				$customercheck["province"] = $row["province"];
				$customercheck["country"] = $row["Country"];
...
else
{
	function editUserAccount($userID,$FirstName,$LastName,$Country,$sex="",$DOB="",$houseNo="",$street="",$area="",$nrcNumber="",$town=""){
		...
		$query = "UPDATE _user_acnts SET FirstName='".$FirstName."',LastName='".$LastName."',sex='".$sex."',DOB='".$DOB."',houseNo='".$houseNo."',street='".$street."',area='".$area."',nrcNumber='".$nrcNumber."',town='".$loc_array["town"]."',province='".$loc_array["province"]."',Country='".$loc_array["country"]."' WHERE (Mobile = '".standadizesMobile($userID)."');";
...
else
{
	function activateUserAccount($userID){
		...
		$insd = "UPDATE _user_acnts SET Active = '1' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
{
	function enableUserShop($userID){
		
		include find_file("cnct.php");

		$insd = "UPDATE _user_acnts SET enableStore = '1' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
{
	function setAccountMobileVerified($userID){
		
		include find_file("cnct.php");

		$insd = "UPDATE _user_acnts SET MobileVerified = '1' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
{
	function setAccountEmailVerified($userID){
		
		include find_file("cnct.php");

		$insd = "UPDATE _user_acnts SET EmailVerified = '1' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function checkUserAccountStatus($userID){
		...
		$insd = "SELECT Active FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
{
	function checkAccountMobileVerified($userID){
		
		include find_file("cnct.php");

		$insd = "SELECT MobileVerified FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
{
	function checkAccountEmailVerified($userID){
		
		include find_file("cnct.php");

		$insd = "SELECT EmailVerified FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function checkLoginNameUsed($userID){
		...
		$insd = "SELECT COUNT(userID) AS numUsed FROM _user_acnts WHERE userID = (SELECT max(userID) FROM _user_acnts WHERE LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function deactivateUserAccount($userID){
		...
		$insd = "UPDATE _user_acnts SET Active = '0' WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function deleteUserAccount($userID){
		...
		$insd = "DELETE FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."');";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function resetAccount($userID){
		...
		$insd = "SELECT * FROM _user_acnts WHERE userID = (SELECT max(userID) FROM _user_acnts WHERE (userID = '".$userID."' OR Email = '".$userID."' OR vCode = '".$userID."' OR Mobile = '".standadizesMobile($userID)."' OR LoginName = '".$userID."'));";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
...
else
{
	function resetPassword($userID,$newPassword){
		...
		$insd = "UPDATE _user_acnts SET Password = '".md5($newPassword)."' WHERE userID = '".$userID."';";
		//echo $insd;
		@$resd = mysqli_query($db,$insd);
	...
	function mobileRecovery($vCode,$userID,$newPassword1,$newPassword2){
		...
		if(verifyAccountVCode($vCode,$userID)==1)
		{
			...
			include find_file("cnct.php");
				...
				$insd = "UPDATE _user_acnts SET Password = '".md5(trim($newPassword))."' WHERE userID = '".$userID."';";
				//echo $insd;
				@$resd = mysqli_query($db,$insd);
