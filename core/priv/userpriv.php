...
else
{
	function priv($levelv="CURRENT",$PageIDv="CURRENT",$typeIn=""){
		...
		$check_permissions = $rows_perm["check_permissions"];
		$admin_permissions = $rows_perm["admin_permissions"];
		
		if($typeIn=="" || $typeIn=="PERMISSIONS")
		{
			$array = array('type'=>$UserTypeNamezz,'category'=>$UserTypeCategoryzz,'limit'=>$LimitedTozz,'check'=>$check_permissions,'admin'=>$admin_permissions,'view'=>$view_permissions,'insert'=>$insert_permissions,'delete'=>$delete_permissions,'edit'=>$edit_permissions,'list'=>$list_permissions,'setup'=>$setup_permissions);
			return $array;
		}
		elseif($typeIn=="LIMIT")
		{
