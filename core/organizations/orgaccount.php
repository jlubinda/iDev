...
if(function_exists('listUsersForOrg'))
{
	
}
else
{
	function listUsersForOrg($userAccount,$DATA_CENTRE="test",$sessID=""){
		
		if(strtolower($DATA_CENTRE)=="live")
		{
			include find_file("cnct.php");
		}
		else
		{
			include find_file("cncttest.php");
		}
		
		
		$sel = "SELECT * FROM meta WHERE id IN (SELECT max(id) FROM meta WHERE userid = (SELECT vCode FROM _organization WHERE id = (SELECT max(id) AS maxID FROM _organization WHERE (id = '".$userAccount."' OR shortname = '".strtolower($userAccount)."' OR phone = '".standadizesMobile($userAccount)."' OR email = '".$userAccount."' OR name like '".$userAccount."' OR reg_number = '".$userAccount."' OR TPIN = '".$userAccount."' OR vCode = '".$userAccount."'))) AND meta_data = '".md5("ORGANIZATION USER")."' GROUP BY userid);";
		//echo $sel."<br><br>";
		...
		for($a=0; $a<$num; $a++)
		{
			...
			$array[] = array('num'=>$num,'vCode'=>$org_vCode,'userID'=>$user,'FirstName'=>getUserData($user,"FirstName",$DATA_CENTRE,$sessID),'LastName'=>getUserData($user,"LastName",$DATA_CENTRE,$sessID),'Mobile'=>getUserData($user,"Mobile",$DATA_CENTRE,$sessID),'userLevel'=>getUserData($user,"level",$DATA_CENTRE,$sessID),'sex'=>getUserData($user,"sex",$DATA_CENTRE,$sessID),'nrcNumber'=>getUserData($user,"nrcNumber",$DATA_CENTRE,$sessID),'Email'=>getUserData($user,"Email",$DATA_CENTRE,$sessID),'level'=>$level);
		}
		return $array;
