...
else 
{
	...
	if($bx_permissions=="Yes")
	{
		echo "<table align='center' width='99%' bgcolor='#fcfcfc' id='tables_css'><tr><td width='30'></td><td width='130'><u><b>Status (iDev Usage)</b></u></td><td><u><b>Organization Details</b></u></td><td width='300'><u><b>Addresses</b></u></td><td><u><b>Contact Details</b></u></td><td><u><b>Country</b></u></td><td width='120'><u><b></b></u></td></tr>";
	...
	$limiter = " WHERE (id = '".$org."')";
			...
			$stat = "SELECT * FROM _bx_organization WHERE id = (select max(id) as maxID from _bx_organization where org_id = '".$resourceTypeID."');";
			@$restat = mysqli_query($db,$stat);
			@$rwstat = mysqli_fetch_array($restat);
			$Status = $rwstat["Status"];
	...
	if($bx_permissions=="Yes")
	{
		...
		{
		$selx = "select * from _bx_organization where org_id = '".$_REQUEST["typeID"]."';";
		@$resx = mysqli_query($db,$selx);
		@$numx = mysqli_num_rows($resx);
		if($numx>0)
		{
		$delete = "UPDATE _bx_organization SET Status = '".$_REQUEST["status"]."' WHERE org_id = '".$_REQUEST["typeID"]."';";
		$res = mysqli_query($db,$delete);
		}
		else
		{
		$delete = "INSERT INTO _bx_organization (id,org_id,recordedby,Status) VALUES ('','".$_REQUEST["typeID"]."','".$userID."','".$_REQUEST["status"]."');";
		$res = mysqli_query($db,$delete);
		}
...
<input type="hidden" name="Misc" value="<?php echo $_REQUEST["Misc"]; ?>">
	...
	<table width="500" align="center" bgcolor='#fcfcfc' id='tables_css'>
		...
		<tr>
			...
			{
			include_once "queries/ins_organization_type.php";
