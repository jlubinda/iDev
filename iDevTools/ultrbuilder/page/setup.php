	...
	@$numD = mysqli_query($db,$resD);
		...
		elseif($_REQUEST["submitBtn"]=="Install Album Style" && (strtolower($fileComp[1])=="php"))
		{
			...
			$cccxA = md5(md5((date(Y)*365*24*60*60)+(date(z)*24*60*60)+(date(G)*60*60)+(date(i)*60)+date(s))." |A1| ".md5(rand(0, 999999999999999999999999))." |B1| ".md5($userID.$namearray)."".md5($fileComp[1].$array)."|C1| ".SesVar()." |D1| ");	
			$cccxB = md5(md5((date(Y)*365*24*60*60)+(date(z)*24*60*60)+(date(G)*60*60)+(date(i)*60)+date(s))." |A2| ".md5(rand(0, 999999999999999999999999))." |B2| ".md5($userID.$namearray)."".md5($fileComp[1].$array)."|C2| ".SesVar()." |D2| ");	
