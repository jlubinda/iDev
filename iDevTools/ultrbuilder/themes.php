...
echo "<div><table id='segment_nav_head'><tr><td align='center'>";
	...
	if(!$_REQUEST["level"])
	{
	$dirxx = "<span class='segment_nav'><a href='?ref=".$_REQUEST['ref']."&segment=".$_REQUEST['segment']."&function=list'>THEMES</a></span>/";
	}
	else
	{
		...
		for($ox=0; $ox<($ov+1); $ox++)
		{
			...
			if(($ov)==0)
			{
			$rootx = "<span class='segment_nav'><a href='?ref=".$_REQUEST['ref']."&segment=".$_REQUEST['segment']."&function=list'>THEMES</a></span>";
			}
			else
			{
			$rootx = "";
...
<?php
if($_REQUEST["ref"]=="3" && $_REQUEST["segment"]=="b1")
{
