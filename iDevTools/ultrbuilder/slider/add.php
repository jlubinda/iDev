	...
	if(($_REQUEST["submitBtn"]=="") || ($_REQUEST["submitBtn"]=="Upload" && !($msg=="")))
	{
		...
		<tr>
			<td><b>Album Type: </b></td><td><select name="Gtype"><option value="Default-Rotator">Default-Rotator</option><option value="Default-Slider">Default-Slider</option><?php
				...
				foreach($namearray as $fileName)
				{
					...
					imagedestroy($src2);
						...
						${$a."res"} = mysqli_query($db,${$a."ins"});
							...
							echo '<div align="center" id="galleryIMG"><table align="center" id="tables_css3"><tr><td align="center"><input name="imgTitle'.$a.'" type="text" size="20" align="center"></td></tr><tr><td align="center"><img src="'.$targetthumb.'" height="120" align="center"></td></tr><tr><td><textarea name="imgComment'.$a.'" title="Comments about this photo" rows="3" cols="25">'.$_REQUEST["imgComment".$a].'</textarea></td></tr></table></div>';
							}
							else
							{
	...
	{
		$ins = "INSERT INTO _sliders (id,name,title,src,Gtype,parent_page,mode,misc,status,uid,org,UGroup,town,province,country) VALUES ('','','".@ eregi_replace("'","`", $_REQUEST["gallery"])."','".$_REQUEST["jdi"]."','".$_REQUEST["Gtype"]."','','".$_REQUEST["Privacy"]."','','Active','".$userID."','".$org."','".$level."','".$userTown."','".$userProvince."','".$userCountry."');";
		$res = mysqli_query($db,$ins);
		if($res)
		{
			if($postV=="")
			...
			{	
				...
				if(!($_REQUEST["imgTitle".$a]==""))
				{
				${$a."title"} = "INSERT INTO meta (id,userid,data,meta_data) VALUES ('','".$_REQUEST["pic".$a]."','".@ eregi_replace("'","`", $_REQUEST["imgTitle".$a])."','".md5("IMAGE-TITLE")."');";
				${$a."resTi"} = mysqli_query($db,${$a."title"});
				$d=$d+1;
					if(${$a."resTi"})
					{
					$e=$e+1;
