	...
	elseif(!($_REQUEST["gid"]==""))
	{
		...
		elseif($_REQUEST["deleteBtn"]=="Delete")
		{
			...
			elseif($_REQUEST["submitBtn"]=="Submit")
			{
				for($a=0; $a<$_REQUEST["count"]; $a++)
				{
					...
					if(@$numI3>="1")
					{
					$update = "UPDATE meta SET data = '".$_REQUEST["imgComment".$a]."' WHERE id = '".$idx2."';";
					}
					else
					{
					...
					else
					{
						...
						if(@$numI4>="1")
						{
						$update = "UPDATE meta SET data = '".$_REQUEST["imgTitle".$a]."' WHERE id = '".$idx2."';";
						}
						else
						{
						$update = "INSERT INTO meta (id,userid,data,meta_data) VALUES ('','".$idx."','".@ eregi_replace("'","`", $_REQUEST["imgTitle".$a])."','".md5("IMAGE-TITLE")."');";
						}
						//echo $update."<br>";
						$resup = mysqli_query($db,$update);
		...
		$msg .= "<p align='center'>No files have been uploaded. Please try again.</p>";
			...
			{
				$update = "UPDATE _sliders SET title = '".@ eregi_replace("'","`", $_REQUEST["gallery"])."' WHERE id = '".$_REQUEST["gid"]."';";
				//echo $update."<br>";
				$resup = mysqli_query($db,$update);
			...
			{	
				$update = "UPDATE _sliders SET mode = '".@ eregi_replace("'","`", $_REQUEST["Privacy"])."' WHERE id = '".$_REQUEST["gid"]."';";
				$resup = mysqli_query($db,$update);
				if($resup)
				{
