	...
	function addVehicleCategory($data,$CatID="DEFAULT"){
		
		include "cnct.php";
		
		$ins = "INSERT INTO meta (id,userid,data,meta_data) VALUES ('','".md5($CatID)."','".$data."','".md5("PORTAL VEHICLE CATEGORIES")."');";
		$res = mysqli_query($db,$ins);
	...
	function listVehicleCategories($CatID="",$max_price=""){
		...
		if($max_price=="")
		{
			if($CatID=="")
			{
				$sel = "SELECT * FROM meta WHERE meta_data = '".md5("PORTAL VEHICLE CATEGORIES")."';";
			}
			else
			{
				$sel = "SELECT * FROM meta WHERE meta_data = '".md5("PORTAL VEHICLE CATEGORIES")."' AND (userid = '".md5($CatID)."' || id = '".$CatID."');";
			}
		}
		else
		{
			if($CatID=="")
			{
				$sel = "SELECT * FROM meta WHERE meta_data = '".md5("PORTAL VEHICLE CATEGORIES")."' AND id IN (SELECT r_user FROM meta WHERE (meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE LOCAL")."' OR meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE OUT OF TOWN")."') AND (numdata < '".$max_price."' AND numdata > '0' AND numdata != ''));";
			}
			else
			{
				$sel = "SELECT * FROM meta WHERE meta_data = '".md5("PORTAL VEHICLE CATEGORIES")."' AND (userid = '".md5($CatID)."' || id = '".$CatID."') AND id IN (SELECT r_user FROM meta WHERE (meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE LOCAL")."' OR meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE OUT OF TOWN")."') AND (numdata < '".$max_price."' AND numdata > '0' AND numdata != ''));";
			}
		}
	...
	function addCategoryPrice($CatID,$price_within,$price_out,$location){
		
		if(addCategoryPriceLocal($CatID,$price_within,$location)==1 && addCategoryPriceOOT($CatID,$price_out,$location)==1)
		{
			return 1;
		}
		else
	...
	function getCategoryPrice($CatID,$location="",$type="LOCAL"){
		...
		//echo $sel." ".$location." ".md5($location)."";
	...
	function deleteCategoryPrice($CatID,$location="",$type="LOCAL"){
		...
		if($location=="")
		{
			$del = "DELETE FROM meta WHERE (meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE OUT OF TOWN")."' || meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE LOCAL")."') AND userid like '".md5($CatID)."|%';";
		}
		else
		{
			$del = "DELETE FROM meta WHERE (meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE OUT OF TOWN")."' || meta_data = '".md5("PORTAL VEHICLE CATEGORY PRICE LOCAL")."') AND userid = '".md5($CatID)."|".md5($location)."';";
		}
	...
	function getPumpPrice($fuelType){
	...
	function listPumpPrices(){
		...
		$array[] = array('id'=>$id,'num'=>$num,'fueltype'=>$userid,'amount'=>$numdata);
		}
	...
	function getLodgingBudget($tocity,$fromcity){
	...
	function getFoodBudget($tocity,$fromcity){
	...
	function getBusFare($tocity,$fromcity){
		...
		{			
			...
			return ($data+getLodgingBudget($tocity,$fromcity)+getFoodBudget($tocity,$fromcity));
		}
		else
		{
			@mysqli_close($db);
	...
	function listBusFares($tocity,$fromcity){
		...
		$array[] = array('id'=>$id,'from'=>$xx[0],'to'=>$xx[1],'amount'=>$numdata,'lodging'=>getLodgingBudget($xx[1],$xx[0]),'food'=>getFoodBudget($xx[1],$xx[0]));
		}
