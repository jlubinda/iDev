	...
	$user = userData();
		...
		?>
		  <div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h3 class="page-header">YOUR COMPANIES | LOGO</h3>
				</div>
			</div>
			<div class="row">
		...
		echo "<form action='' method='post' class='motion' enctype='multipart/form-data'>Logo: <input type='file' name='productImage' size='20' accept='image/jpeg'><input type='Submit' name='submitBtn' size='20' value='UPLOAD'></form>";
		...
		$user = userData();
			...
			if($_FILES['productImage']['name'])
			{
				...
				$name1 = "profilepics/".basename( $_FILES['productImage']['name']);	
  ...
  ?><!-- @end #preview-pane -->
	...
	<form id="cropimg" name="cropimg" action="<?php echo '?ref='.$_REQUEST["ref"].'&segment='.$_REQUEST["segment"].'&unit='.$_REQUEST["unit"].'&class='.$_REQUEST["class"].'&Programme='.$_REQUEST["Programme"].'&product='.$_REQUEST["product"].'&vCode='.$_REQUEST["vCode"].'&id='.$_REQUEST["id"].'&task=Enter_Data';?>" method="post" onsubmit="return checkCoords();">
		...
		<input type="submit" value="Crop Image" class="btn btn-large btn-inverse" name="cropBtn"/>
					...
					{
					echo "<p align='center'>Error! Your logo could not be uploaded. Please try again later.</p>";
					}
				}
				else 
				{
			...
			{		
				...
				$ft = explode("profilepics/",$src);
				$thumb = $ft[0]."profilepics/thumb_".$ft[1];
				...
				if($_REQUEST["x2"]=="IMG")
				{
					...
					$array = addOrgProfilePicture($_REQUEST["vCode"],$image);
					if($array==1)
					{
					echo "<img src='".$thumb."' align='center'>";
					echo "<p align='center'>Success! Your logo has been updated.</p>";
					}
					else
					{
					echo "<p align='center'>Error! Your logo was not updated. Please try again later.</p>";
					}
				}
				//exit;
			}
