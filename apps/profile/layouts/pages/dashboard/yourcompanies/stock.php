<?php
	...
	{
?>
				<div class="col-md-6 agile_about_grid_left" style="max-width:250px;">
		...
		for($a=0; $a<$cats[0]['num']; $a++)
		{
			...
			?>
				...
				for(${"a".$e."_".$a}=0; ${"a".$e."_".$a}<$_REQUEST["products".$a][0]["num"]; ${"a".$e."_".$a}++)
				{
					...
					//{
					?>
						<div class="col-md-3 top_brand_left" style="margin-bottom:<?php echo $xx;?>px; min-width:200px;">
							<a href="?ref=<?php echo $_REQUEST["ref"];?>&class=<?php echo $cats[$a]['class'];?>&Programme=<?php echo $cats[$a]['name'];?>&product=<?php echo $_REQUEST["products".$a][${"a".$e."_".$a}]["id"];?>&vCode=<?php echo $_REQUEST["vCode"];?>">
										...
										<figure>
											<div class="snipcart-item block" >
												<div class="snipcart-thumb" style="min-height:100px;">
													<?php 
													$qtyx = checkProductInUserStore($_REQUEST["products".$a][${"a".$e."_".$a}]["id"],$vCode,"QUANTITY");
													
													if($qtyx>=1)
													{
														$unitIDx = checkProductInUserStore($_REQUEST["products".$a][${"a".$e."_".$a}]["id"],$vCode,"UNIT");
														
														echo "<p>You have ".$qtyx." ".getUnit($unitIDx)." of ".$_REQUEST["products".$a][${"a".$e."_".$a}]["name"]." @ K".checkProductInUserStore($_REQUEST["products".$a][${"a".$e."_".$a}]["id"],$vCode,"PRICE")." each in your shop.</p>";
														$funcx = "Update";
														$fx = "in";
													}
													else
		...
		<input type="submit" value="Crop Image" class="btn btn-large btn-inverse" name="cropBtn"/>
			...
			{		
				...
				if($_REQUEST["x2"]=="IMG")
				{
					...
					$array = addOrgProductForSale($_REQUEST["vCode"],$_REQUEST["product"],$_REQUEST["price"],$_REQUEST["qty"],$_REQUEST["pUnit"],$transporter,$image);
					if($array["success"]==1)
					{
