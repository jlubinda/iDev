		...
		$limiter = "WHERE ".$org_limiter3;
			...
			//echo "Active Status: ".$Active;
					...
					else
					{
						...
						$active_link = "<a href='?ref=settings&segment=e3&function=edit&unit=8&pvCode=".$_REQUEST["pvCode"]."&typeID=".$userID."&typeName=".$typeName."&Active=".$function1."&proceedbtn=Proceed'>".$wordin1."</a>";
...
country[<?php echo $y4; ?>] = "<?php echo $_country4; ?>";
	...
	?>
		...
		<tr>
			...
			if($_REQUEST["level"])
			{
				$selectLevelc = "SELECT * FROM refer_users_types WHERE UserTypeCode = '".$_REQUEST["level"]."';";
	...
	$limiter = " WHERE (LimitedTo = 'User`s Branch Only')";
					...
					$select_user_type = "SELECT * FROM refer_users_types".$limiter.";";
...
?>
		...
		<tr>
			...
			{
				$selectLevelc = "SELECT * FROM refer_users_types WHERE UserTypeCode = '".$typeCodex."';";
				@$resLevelc = mysqli_query($db,$selectLevelc);
				$rowLevelc = mysqli_fetch_array($resLevelc);
				$LevelCodec = $rowLevelc["UserTypeName"];
...
?>
		...
		<tr>
			...
			elseif($_REQUEST["proceedbtn"]=="Submit")
			{
				...
				$insd = "UPDATE _user_acnts SET Active = '".$_REQUEST["Active"]."' WHERE userID = '".$_REQUEST["typeID"]."';";
				$resd = mysqli_query($db,$insd);
