 ...
 if(@ privacy('Secure|Priv')=='Granted')
{
		...
		echo "<table align='center' width='100%'  id='tables_css'><tr><td width='30'></td><td width='150'><u><b>Page ID</b></u></td><td><u><b>Name</b></u></td><td><u><b>Description</b></u></td><td width='40'><u><b></b></u></td><td width='50'><u><b></b></u></td></tr>";
			$sel = "SELECT * FROM sys_pages ORDER BY PageID;";
	...
	$bgcolor = "ddffff";
		...
		elseif($_REQUEST['ref']=="settings" && $_REQUEST['segment']=="h1" && $_REQUEST['function']=="delete" && $_REQUEST['unit']=="2" && !$_REQUEST['submitBtn'] && $_REQUEST["typeID"])
		{
			...
			<table align="center" width="800"  id='tables_css'>
				<tr>
		...
		{
		$delete = "DELETE FROM sys_pages WHERE id = '".$_REQUEST["typeID"]."';";
		...
		<?php
			...
			$query_test = "select max(id) as max_ID from sys_pages where PageID = '".$_REQUEST["PageID"]."';";
			$result_test = mysqli_query($db,$query_test);
			@$row_test = mysqli_fetch_array($result_test);
			@$max_ID = $row_test["max_ID"];
		
				
			$sql21 = "INSERT INTO sys_pages (id,PageID,name,description,misc) VALUES ('','".$_REQUEST["PageID"]."','".$_REQUEST["name"]."','".$_REQUEST["description"]."','".$_REQUEST["misc"]."');";
		...
		<?php
			...
			$sql21 = "UPDATE sys_pages SET name = '".$_REQUEST["name"]."',description = '".$_REQUEST["description"]."',misc = '".$_REQUEST["misc"]."' WHERE PageID = '".$_REQUEST["PageID"]."';";
			$res21 = mysqli_query($db,$sql21);
					if($res21)
					{
